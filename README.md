...
![dsh-pet 效果演示](docs/screenshot.png)

右下角的鲸鱼娘女仆长常驻挂件，点击「监督观察对话」后会读取当前会话的转写和工具调用结果，生成带证据的吐槽气泡，并附「复制修正 Prompt」按钮——图中就是她抓到助手嘴上说"打包完成 / 已包含 / 已打进"却拿不出对应的真实改动或文件操作证据的例子。
